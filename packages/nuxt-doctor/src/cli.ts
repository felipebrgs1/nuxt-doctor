...
import type { Diagnostic, DiffInfo, FailOnLevel, NuxtDoctorConfig, ScanOptions } from "./types.js";
...
const program = new Command()
  .name("nuxt-doctor")
  .description("Diagnose Vue/Nuxt codebase health")
  ...
  .action(async (directory: string, flags: CliFlags) => {
    ...
    try {
      ...
      if (flags.staged) {
        ...
        const tempDirectory = mkdtempSync(path.join(tmpdir(), "nuxt-doctor-staged-"));
...
program
  .command("install")
  .description("Install the nuxt-doctor skill into your coding agents")
