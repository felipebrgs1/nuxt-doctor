...
const buildBrandingLines = (
  ...
  if (scoreResult) {
    ...
    lines.push(
      createFramedLine(
        "Nuxt Doctor (www.nuxt.doctor)",
        `Nuxt Doctor ${highlighter.dim("(www.nuxt.doctor)")}`,
      ),
    );
...
const printProjectDetection = (
  ...
  completeStep(
    `Detecting Vue SFC compiler. ${projectInfo.hasVueCompilerSfc ? highlighter.info("Found Vue SFC compiler.") : "Not found."}`,
  );
  ...
  if (userConfig) {
    completeStep(`Loaded ${highlighter.info("nuxt-doctor config")}.`);
  }
        ...
        try {
          ...
          if (!options.scoreOnly) {
            ...
            if (isNativeBindingError) {
              ...
              logger.dim(
                `  Upgrade to Node ${OXLINT_NODE_REQUIREMENT} or run: npx -p oxlint@latest nuxt-doctor@latest`,
              );
            } else {
