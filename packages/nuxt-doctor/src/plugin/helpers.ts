import {
  FETCH_CALLEE_NAMES, FETCH_MEMBER_OBJECTS, LOOP_TYPES,
  SETTER_PATTERN, UPPERCASE_PATTERN,
} from "./constants.js";
...
export const isSetterCall = (node: EsTreeNode): boolean =>
  node.type === "CallExpression" && node.callee?.type === "Identifier" && isSetterIdentifier(node.callee.name);
...
export const isMemberProperty = (node: EsTreeNode, propertyName: string): boolean =>
  node.type === "MemberExpression" && node.property?.type === "Identifier" && node.property.name === propertyName;
...
export const isSimpleExpression = (node: EsTreeNode | null): boolean => {
  ...
  switch (node.type) {
    case "Identifier": case "Literal": case "TemplateLiteral": return true;
...
export const isComponentAssignment = (node: EsTreeNode): boolean =>
  ...
  isUppercaseName(node.id.name) && Boolean(node.init) &&
...
export const hasDirective = (programNode: EsTreeNode, directive: string): boolean =>
  Boolean(programNode.body?.some(
    (statement: EsTreeNode) => statement.type === "ExpressionStatement" && statement.expression?.type === "Literal" && statement.expression.value === directive,
  ));

export const hasUseServerDirective = (node: EsTreeNode): boolean => {
  if (node.body?.type !== "BlockStatement") return false;
  return Boolean(node.body.body?.some(
    (statement: EsTreeNode) => statement.type === "ExpressionStatement" && statement.directive === "use server",
  ));
};
...
export const findJsxAttribute = (attributes: EsTreeNode[], attributeName: string): EsTreeNode | undefined =>
  attributes?.find(
    (attr: EsTreeNode) => attr.type === "JSXAttribute" && attr.name?.type === "JSXIdentifier" && attr.name.name === attributeName,
  );
...
export const createLoopAwareVisitors = (innerVisitors: Record<string, (node: EsTreeNode) => void>): RuleVisitors => {
  let loopDepth = 0;
  const incrementLoopDepth = () => { loopDepth++; };
  const decrementLoopDepth = () => { loopDepth--; };
  ...
  for (const [nodeType, handler] of Object.entries(innerVisitors)) {
    visitors[nodeType] = (node: EsTreeNode) => { if (loopDepth > 0) handler(node); };
  }
  return visitors;
};
...
export const findSideEffect = (node: EsTreeNode): string | null => {
  ...
  walkAst(node, (child: EsTreeNode) => {
    ...
    if (child.type === "AssignmentExpression" && child.left?.type === "MemberExpression") {
      const objectName = child.left.object?.type === "Identifier" ? child.left.object.name : null;
      if (objectName === "process" && child.left.property?.type === "Identifier" && child.left.property.name === "env") { sideEffect = "process.env mutation"; }
    }
  });
  return sideEffect;
};
  ...
  for (const param of params) {
    if (param.type === "ObjectPattern") {
      for (const property of param.properties ?? []) {
        if (property.type === "Property" && property.key?.type === "Identifier") propNames.add(property.key.name);
      }
